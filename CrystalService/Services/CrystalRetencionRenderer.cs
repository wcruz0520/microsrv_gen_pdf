...
{
    public class CrystalRetencionRenderer
    {
        private readonly string _rptPath;
        private readonly string _assetsDir;
        private readonly string _tempDir;

        public CrystalRetencionRenderer(string rptPath, string assetsDir, string tempDir)
        {
            _rptPath = rptPath;
        ...
        public byte[] RenderRetencion(RetencionRenderRequest req)
        {
            if (!File.Exists(_rptPath))
                throw new FileNotFoundException("No existe el .rpt", _rptPath);

            DataSet dsNc = new DataSet();
            dsNc.ReadXmlSchema(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Fomatos", "Retencion.xsd"));

            // 1) Dataset
            DataSet ds = RetencionDataSetBuilder.FillRetencionDataSet(dsNc, req);
