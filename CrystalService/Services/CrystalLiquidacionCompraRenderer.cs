...
{
    public class CrystalLiquidacionCompraRenderer
    {
        private readonly string _rptPath;
        private readonly string _assetsDir;
        private readonly string _tempDir;

        public CrystalLiquidacionCompraRenderer(string rptPath, string assetsDir, string tempDir)
        {
            _rptPath = rptPath;
        ...
        public byte[] RenderLiquidacionCompra(LiquidacionCompraRenderRequest req)
        {
            if (!File.Exists(_rptPath))
                throw new FileNotFoundException("No existe el .rpt", _rptPath);

            DataSet dsNc = new DataSet();
            dsNc.ReadXmlSchema(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Fomatos", "LiquidacionCompra.xsd"));

            // 1) Dataset
            DataSet ds = LiquidacionCompraDataSetBuilder.FillLiquidacionCompraDataSet(dsNc, req);
