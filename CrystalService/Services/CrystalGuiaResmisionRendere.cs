...
{
    public class CrystalGuiaRemisionRenderer
    {
        private readonly string _rptPath;
        private readonly string _assetsDir;
        private readonly string _tempDir;

        public CrystalGuiaRemisionRenderer(string rptPath, string assetsDir, string tempDir)
        {
            _rptPath = rptPath;
        ...
        public byte[] RenderGuiaRemision(GuiaRemisionRenderRequest req)
        {
            if (!File.Exists(_rptPath))
                throw new FileNotFoundException("No existe el .rpt", _rptPath);

            DataSet dsNc = new DataSet();
            dsNc.ReadXmlSchema(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Fomatos", "GuiaRemision.xsd"));

            // 1) Dataset
            DataSet ds = GuiaRemisionDataSetBuilder.FillGuiaRemisionDataSet(dsNc, req);
